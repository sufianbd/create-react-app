...
<div class="page">
  <div class="header">
    <h1>{{ $company }}</h1>
    <div class="number">Bill #{{ $bill->number }}</div>
  </div>
  ...
  <table class="meta-table">
    ...
    <tr>
      ...
      <td><span class="badge badge-{{ $bill->status }}">{{ strtoupper($bill->status) }}</span></td>
    </tr>
  </table>
  ...
  <table class="totals-table">
    ...
    <tr><td>Tax:</td><td class="text-right">{{ number_format($bill->tax_total, 2) }}</td></tr>
    <tr class="total-row"><td>Total:</td><td class="text-right">{{ number_format($bill->total, 2) }}</td></tr>
    <tr><td>Amount Paid:</td><td class="text-right">{{ number_format($bill->amount_paid, 2) }}</td></tr>
    <tr><td>Amount Due:</td><td class="text-right"><strong>{{ number_format($bill->amount_due, 2) }}</strong></td></tr>
  </table>

  @if($bill->notes)
  <p style="margin-top:16px;color:#64748b;">{{ $bill->notes }}</p>
  @endif
