...
export function NotificationBell() {
    const { notifications_count } = usePage<PageProps & { notifications_count: number }>().props;
            ...
            {open && (
                <div className="absolute right-0 top-10 z-50 w-80 rounded-xl border border-slate-200 bg-white shadow-lg">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                        ...
                        {notifications_count > 0 && (
                            <button
                                onClick={() => { router.patch('/notifications/read-all'); setOpen(false); }}
                    ...
                    {notifications_count === 0 ? (
                        ...
                        <div className="max-h-72 overflow-y-auto">
                            <p className="px-4 py-3 text-xs text-slate-500">
                                {notifications_count} unread notification{notifications_count !== 1 ? 's' : ''}
                            </p>
                        </div>
                    )}
