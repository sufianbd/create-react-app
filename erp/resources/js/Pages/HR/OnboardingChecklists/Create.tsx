...
export default function OnboardingChecklistsCreate(_props: PageProps) {
    ...
    function updateTask(index: number, field: keyof TaskRow, value: string | number | boolean) {
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        post('/hr/onboarding-checklists');
    }

    return (
        <AppLayout>
            <Head title="New Onboarding Checklist" />
            <div className="max-w-3xl space-y-6">
                <div>
                    <h1 className="text-2xl font-semibold text-slate-900">New Onboarding Checklist</h1>
                </div>

                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="rounded-lg border border-slate-200 bg-white shadow-sm p-6 space-y-4">
                        <h2 className="text-base font-medium text-slate-900">Checklist Details</h2>
                                        ...
                                        <div className="flex items-start justify-between gap-3">
                                            <div className="flex-1 space-y-3">
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    ...
                                                    <div>
                                                        <label className="block text-xs font-medium text-slate-600 mb-1">Due Day Offset</label>
                                                        <input
                                                            type="number"
                                                            min={0}
                                                            value={task.due_day_offset}
                                                            onChange={(e) => updateTask(index, 'due_day_offset', parseInt(e.target.value) || 0)}
                                                            className="block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                                        />
                                                    </div>
                                                </div>
