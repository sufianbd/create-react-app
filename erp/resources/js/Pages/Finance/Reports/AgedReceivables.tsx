...
export default function AgedReceivables({ rows, totals, grand_total, as_of }: Props) {
        ...
        <AppLayout>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-semibold text-slate-900">Aged Receivables</h1>
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => router.get('/finance/reports/aged-receivables', { as_of: asOf }, { preserveState: true, replace: true })}
                ...
                <div className="rounded-lg border border-slate-200 bg-white shadow-sm overflow-hidden">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {rows.length === 0 && (
                                <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-slate-400">No outstanding invoices.</td></tr>
                            )}
                            {rows.map((row) => (
                                <tr key={row.id} className="hover:bg-slate-50">
                                    <td className="px-4 py-3 font-mono text-indigo-600">
                                        <a href={`/finance/invoices/${row.id}`} className="hover:underline">{row.number ?? `#${row.id}`}</a>
                                    </td>
