...
export default function EditOrder({ order, users }: Props) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/field-service/orders/${order.id}`);
    };

    return (
        <AppLayout>
            <Head title={`Edit ${order.order_number ?? 'Order'}`} />
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    <Link href={`/field-service/orders/${order.id}`} className="text-sm text-slate-500 hover:text-slate-700">&larr; Back</Link>
                    <h1 className="text-2xl font-semibold text-slate-900">Edit Order {order.order_number}</h1>
                </div>
                ...
                <form onSubmit={submit} className="space-y-6">
                    <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select className="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm" value={data.status} onChange={(e) => setData('status', e.target.value)}>
                                        ...
                                        {data.items.map((item, i) => (
                                            <tr key={i}>
                                                <td className="py-2 pr-4">
                                                    <input type="text" className="w-full rounded border border-slate-300 px-2 py-1 text-sm" value={item.description} onChange={(e) => updateItem(i, 'description', e.target.value)} />
                                                </td>
                                                <td className="py-2 pr-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Link href={`/field-service/orders/${order.id}`}><Button type="button" variant="secondary">Cancel</Button></Link>
                    </div>
                </form>
            </div>
        </AppLayout>
