...
export default function Create({ suppliers, products }: Props) {
    const { data, setData, post, processing, errors } = useForm<{
        supplier_id: string; order_date: string; expected_date: string; currency: string; notes: string;
        items: LineItem[];
    }>({ supplier_id: '', order_date: '', expected_date: '', currency: 'USD', notes: '', items: [{ description: '', quantity: '1', unit_price: '0', product_id: '' }] });
    ...
    return (
        <AppLayout>
            ...
            <div className="p-6 max-w-3xl">
                ...
                <form onSubmit={e => { e.preventDefault(); post('/inventory/purchase-orders'); }}>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            <label className="block text-sm font-medium mb-1">Supplier</label>
                            <select value={data.supplier_id} onChange={e => setData('supplier_id', e.target.value)} className="w-full border rounded px-3 py-2">
